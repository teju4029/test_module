...
class HrPublicHolidays(models.Model):
    ...
    c_no_of_pub_holidays = fields.Integer(string='Num Of Public Holidays')
    ...
    def is_public_holiday(self, selected_date, employee_id=None):
        ...
        if holidays_lines and len(holidays_lines.filtered(
            ...
            return True
        return False
    @api.onchange('line_ids','c_no_of_pub_holidays')
    def counting_no_of_lines(self):
        count = 0
        for val in self.line_ids:
            count = count + 1
        self.c_no_of_pub_holidays = count
